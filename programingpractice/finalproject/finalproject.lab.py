...
def transition_highscore(widget):
    ...
    for i in range(len(scorelist)):
        highscore+=str(i+1)+"位は"+str(scorelist[i])+"点です"+"\n"

    high_canvas = tk.Canvas(background=bg1, width=screenwidth, height=screenheight)
    high_canvas.place(x=0, y=0)
    
    label1 = tk.Label(high_canvas, text = "ハイスコアリスト", font=('',28),bg = '#3775ff', fg = 'black')
    label1.place(relx=0.5, rely=0.1, relwidth=0.5, relheight=0.2, anchor = tk.CENTER)

    label2 = tk.Label(high_canvas, font=("",27), text=highscore)
    label2.place(relx=0.5, rely=0.5,relwidth = 0.6, relheight = 0.6, anchor=tk.CENTER)   
